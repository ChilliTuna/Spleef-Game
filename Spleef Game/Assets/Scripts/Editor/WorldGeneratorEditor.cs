...
public class WorldGeneratorEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Space(10f);
        if(GUILayout.Button(new GUIContent("Generate Static Platform", "Use button to manually generate a static platform with the above details." +
        ...
        if(GUILayout.Button(new GUIContent("Generate Dynamic Platform", "Use button to manually generate a dynamic platform with the above details. There can only be one dynamic platform at any point in time.")))
        {
            WorldGenerator worldGenerator = (WorldGenerator)target;
            worldGenerator.Initialise();
        }
        if(GUILayout.Button(new GUIContent("Remove Dynamic Platform", "Use button to remove the dynamic platform.")))
        {
            WorldGenerator worldGenerator = (WorldGenerator)target;
            worldGenerator.DeletePlatform();
        }
        if (GUILayout.Button(new GUIContent("Staticize Dynamic Platform", "Use button to make dynamic platform static. " +
